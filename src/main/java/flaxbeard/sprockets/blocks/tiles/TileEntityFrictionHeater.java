...
public class TileEntityFrictionHeater extends TileEntitySprocketBase implements IMechanicalConsumer
{
	...
	public void updateParentPlace()
	{
		...
		for (int i = 0; i < facing; i++)
		{
			...
			if (heat instanceof TileEntityFrictionHeater)
			{
				TileEntityFrictionHeater fH = (TileEntityFrictionHeater) heat;
				if (fH.isParent == 1)
				{
					fH.isParent = 0;
					this.isParent = 1;
			...
			if (te != null && te instanceof TileEntityFurnace)
			{
				if (isParent == 1)
				{
					...
					for (int i = 0; i < 6; i++)
					{
						...
						{
							BlockPos otherHeaterPos = furn.subtract(EnumFacing.VALUES[i].getDirectionVec());
							TileEntityFrictionHeater heat = (TileEntityFrictionHeater) worldObj.getTileEntity(otherHeaterPos);
					...
					if (speed > 0)
					{
						int num = Math.max(2, (int) (30. / Math.sqrt(speed * 2)));
						TileEntityFurnace furnaceTE = (TileEntityFurnace) te;
						ticksExisted = (ticksExisted + 1) % num;
						
						if (!worldObj.isRemote)
						{
							if (ticksExisted == 0 && furnaceTE.getField(0) > 0)
							{
								furnaceTE.setField(0, furnaceTE.getField(0) + 1); // increase burn time
							}
						}
						else
						{
	...
	{
		if (facing == -1 && worldObj != null && worldObj.getBlockState(getPosMC()) != null && worldObj.getBlockState(getPosMC()).getBlock() == SprocketsBlocks.frictionHeater)
		{
			facing = SprocketsBlocks.frictionHeater.getMetaFromState(worldObj.getBlockState(getPosMC()));
		}
